...
type BracketsViewerParticipant = { id: number; name: string; tournament_id: number };
...
type BracketsViewerRound   = { id: number; number: number; stage_id: number; group_id: number };
type BracketsViewerGroup   = { id: number; number: number; stage_id: number };
type BracketsViewerStage   = {
  id: number; name: string; number: number; tournament_id: number;
...
function cleanViewerData(data: BracketsViewerData): CleanedData {
  const match = (data.match ?? []).filter(
    (m): m is BracketsViewerMatch => m != null && typeof (m as { id?: number }).id === "number",
  );
  const round = (data.round ?? []).filter(
    (r): r is BracketsViewerRound =>
      r != null && typeof (r as { id?: number }).id === "number" && "stage_id" in r && "group_id" in r,
  );
  const group = (data.group ?? []).filter(
    (g): g is BracketsViewerGroup =>
      g != null && typeof (g as { id?: number }).id === "number" && "stage_id" in g,
  );
  return { stage: data.stage ?? [], group, round, match, match_game: [], participant: data.participant ?? [] };
}
...
function MatchPanel({ match, participants, isDark, onClose }: MatchPanelProps) {
  ...
  const findName = (id: number | null) =>
    id == null ? "TBD" : (participants.find((p) => p.id === id)?.name ?? `Player #${id}`);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: muted, letterSpacing: "0.08em" }}>
          MATCH #{match.id} · ROUND {match.round_id}
        </span>
        <button
          onClick={onClose}
          style={{ background: "none", border: "none", cursor: "pointer", color: muted, fontSize: "18px", lineHeight: 1, padding: "0 2px" }}
        >
          ×
        </button>
      </div>
      ...
      <div style={{ marginBottom: "16px" }}>
        <span style={{
          ...
          fontSize: "11px", fontWeight: 600, color: dotColor, letterSpacing: "0.05em"
        }}>
      ...
      {[match.opponent1, match.opponent2].map((opp, i) => {
        ...
        const rowBg = isWin
          ? (isDark ? "#0c2337" : "#eff6ff")
          : isLoss
          ? (isDark ? "#1f1215" : "#fafafa")
          : cardBg;
        const rowBorder = isWin
          ? "#0ea5e9"
          : isLoss
          ? (isDark ? "#3f1827" : "#e2e8f0")
          : border;
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "11px", color: muted, fontFamily: "'DM Mono', monospace" }}>
                {i === 0 ? "P1" : "P2"}
              </span>
              <span style={{ fontWeight: 500, fontSize: "14px", color: nameColor }}>
                {isForfeit ? `${name} (BYE)` : name}
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              {opp.score != null && (
                <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700, fontSize: "16px", color: isWin ? "#38bdf8" : muted }}>
                  {opp.score}
                </span>
              )}
              {isWin && <span style={{ fontSize: "12px", color: "#22c55e", fontWeight: 700 }}>W</span>}
              {isLoss && <span style={{ fontSize: "12px", color: "#ef4444", fontWeight: 700 }}>L</span>}
            </div>
          </div>
        );
      })}
      ...
      <div style={{
        ...
        {[
          ...
          <div key={label}>
            <div style={{ fontSize: "10px", color: muted, letterSpacing: "0.06em", marginBottom: "2px" }}>{label.toUpperCase()}</div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: "13px", color: text }}>{val}</div>
          </div>
        ))}
      </div>
    </div>
...
const VIEWER_CONFIGS: Record<TournamentTabKey, {
  selector: string;
  ...
  double:      { selector: "#double-elim-viewer",  showSlotsOrigin: true, showLowerBracketSlotsOrigin: true },
...
export default function Home() {
  ...
  const [selectedMatch, setSelectedMatch] = useState<BracketsViewerMatch | null>(null);
  const styleTagRef                       = useRef<HTMLStyleElement | null>(null);

  // ── Data memos ──────────────────────────────────────────────────────────────

  const dataMap = useMemo<Record<TournamentTabKey, CleanedData>>(() => ({
    single:      cleanViewerData(matchesData.single_elimination as unknown as BracketsViewerData),
    double:      cleanViewerData(matchesData.double_elimination as unknown as BracketsViewerData),
    pool:        cleanViewerData(matchesData.Pool_play as unknown as BracketsViewerData),
    round_robin: cleanViewerData(matchesData.round_robin as unknown as BracketsViewerData),
    swiss:       cleanViewerData(matchesData.Swiss as unknown as BracketsViewerData),
  }), []);
  ...
  useEffect(() => {
    ...
    styleTagRef.current.textContent = (isDark ? DARK_CSS : LIGHT_CSS) + EXTRA_CSS;
  }, [isDark]);

  // ── Render bracket ─────────────────────────────────────────────────────────

  const renderBracket = useCallback((tab: TournamentTabKey) => {
    ...
    setTimeout(() => {
      ...
      container.querySelectorAll<HTMLElement>(".match").forEach((el) => {
        const matchIdAttr = el.getAttribute("data-match-id") ??
        ...
        const isDisabled  = match.status === 0 || match.status === 1 || match.status === 4 || match.status === 5;
        ...
        // Status tooltip
        el.setAttribute("title", `Match #${match.id} · ${STATUS_LABEL[match.status] ?? "Unknown"}`);
      });
    }, 150);
  }, [dataMap]);
  ...
  const th = isDark
    ? {
        pageBg:      "linear-gradient(135deg, #020817 0%, #0a1628 50%, #020c1b 100%)",
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "-80px", left: "-80px", width: "400px", height: "400px", borderRadius: "50%", background: th.glowA, filter: "blur(80px)" }} />
        <div style={{ position: "absolute", bottom: "-60px", right: "-60px", width: "350px", height: "350px", borderRadius: "50%", background: th.glowB, filter: "blur(60px)" }} />
      </div>

      <main style={{ position: "relative", zIndex: 1, maxWidth: "1280px", margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
              ...
              <h1 style={{ margin: 0, fontSize: "clamp(20px, 3vw, 28px)", fontWeight: 700, letterSpacing: "-0.02em", color: th.text }}>
                Tournament Brackets
              </h1>
            </div>
            <p style={{ margin: 0, fontSize: "13px", color: th.subText, maxWidth: "480px", lineHeight: 1.5 }}>
              Powered by{" "}
              <code style={{ fontFamily: "'DM Mono', monospace", color: th.accent, fontSize: "12px" }}>brackets-viewer</code>
              {" "}·{" "}Click <span style={{ color: "#22c55e", fontWeight: 600 }}>Ready</span> or <span style={{ color: "#3b82f6", fontWeight: 600 }}>Running</span> matches to inspect them.
            </p>
          </div>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: th.subText }}>
        ...
        <nav style={{
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                key={tab.key}
                onClick={() => { setActiveTab(tab.key); setSelectedMatch(null); }}
                style={{
                  flex: "1 1 auto",
        ...
        <section style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "18px" }}>{TABS.find((t) => t.key === activeTab)?.icon}</span>
              <h2 style={{ margin: 0, fontSize: "15px", fontWeight: 600, color: th.text }}>
                {TABS.find((t) => t.key === activeTab)?.label}
              </h2>
            </div>
            <div style={{ display: "flex", gap: "12px", fontSize: "12px", color: th.subText }}>
              <span>
                <span style={{ color: th.accent, fontFamily: "'DM Mono', monospace", fontWeight: 600 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <div id="single-elim-viewer"  className="brackets-viewer" style={{ display: activeTab === "single"       ? "block" : "none", minWidth: "600px" }} />
            <div id="double-elim-viewer"  className="brackets-viewer" style={{ display: activeTab === "double"       ? "block" : "none", minWidth: "900px" }} />
            <div id="pool-viewer"         className="brackets-viewer" style={{ display: activeTab === "pool"         ? "block" : "none", minWidth: "600px" }} />
            <div id="round-robin-viewer"  className="brackets-viewer" style={{ display: activeTab === "round_robin"  ? "block" : "none", minWidth: "600px" }} />
            <div id="swiss-viewer"        className="brackets-viewer" style={{ display: activeTab === "swiss"        ? "block" : "none", minWidth: "600px" }} />
          </div>
        </section>
      </main>
